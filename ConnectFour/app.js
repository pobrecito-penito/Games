...
const addPlayers = document.querySelector('#add-btn');
const result = document.querySelector('#result');
const squares = document.querySelectorAll('.grid div');
const p = document.querySelector('#cant');
const restart = document.querySelector('#restart');
let currentPlayer = 1;

let players = [{player: 1, name: 'Player 1'},{player: 2, name: 'Player 2'}];
...
addPlayers.addEventListener('click',() => {
    ...
    players.push({player: 1, name: player1});
    players.push({player: 2, name: player2});
    displayCurrentPlayer.innerHTML = `The current Player is: ${players[0].name}`;
})
...
for(let i = 0; i < squares.length; i++){
    squares[i].addEventListener('click',() => {
        if(squares[i].classList.contains('taken')){
           return p.textContent = `It's taken!`
        }
        if(squares[i + 7].classList.contains('taken')){
            ...
            if(currentPlayer === 1){
                ...
                currentPlayer = 2;
                displayCurrentPlayer.innerHTML = `The current Player is: ${players[1].name}`;
        ...
        } else p.textContent = `You can't go here!`
    })
}
...
const checkBoard = () => {
    ...
    for(let j = 0; j < winningArrays.length; j++){
        ...
        if(square1.classList.contains('player-one') &&
                ...
                displayCurrentPlayer.innerHTML = `${players[0].name} wins!`;
                squares.forEach(el => el.classList.add('taken'));
                p.innerHTML = '';
        }
                ...
                displayCurrentPlayer.innerHTML = `${players[1].name} wins!`;
                squares.forEach(el => el.classList.add('taken'));
                p.innerHTML = '';
        }
    }
...
squares.forEach(el => el.addEventListener('click',checkBoard));

restart.addEventListener('click',() => {
    for(let i = 0; i < squares.length - 7; i++){
        if(squares[i].classList.contains('taken')){
            squares[i].classList.value = '';
            squares[i].classList.add('div');
            currentPlayer = 1;
            displayCurrentPlayer.innerHTML = `The current Player is: ${players[0].name}`;
        }
    }
